...
public class TvodVitality {
	...
	public static void main(String[] args)
		...
		String outputPath = "hdfs://" + option.getHdfsActiveHost() + ":" + option.getPort() + "/utsc/output/tvod_vitality";
		...
		Job job01 = new Job(conf01, "tvod_vitality_01");
		ControlledJob jobCtrl01 = new ControlledJob(conf01);

		jobCtrl01.setJob(job01);

		job01.setJarByClass(TvodVitality.class);

		job01.setMapperClass(TvodVitalityMapper1.class);
		job01.setReducerClass(TvodVitalityReducer1.class);
		...
		FileOutputFormat.setOutputPath(job01, new Path(outputPath + "/tvod_vitality_01"));
		...
		Job job02 = new Job(conf02, "tvod_vitality_02");
		ControlledJob jobCtrl02 = new ControlledJob(conf02);

		jobCtrl02.setJob(job02);
		jobCtrl02.addDependingJob(jobCtrl01);

		job02.setJarByClass(TvodVitality.class);

		job02.setMapperClass(TvodVitalityMapper2.class);
		job02.setReducerClass(TvodVitalityReducer2.class);
		...
		FileInputFormat.addInputPath(job02, new Path(outputPath + "/tvod_vitality_01"));

		FileOutputFormat.setOutputPath(job02, new Path(outputPath + "/tvod_vitality_02"));
